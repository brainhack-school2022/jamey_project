...
s13_file_4D = "sub-13_ses-01_task-rest_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz"
s13_file_3D = "sub-13_space-MNI152NLin2009cAsym_desc-preproc_T1w.nii.gz"


sub13_01_3D = os.path.join(a_path, a_folder, s13_file_3D)
fmri_filepath = os.path.join(a_path, a_folder, s13_file_4D)

# path to Nifti file
print('Path to data: %r' % sub13_01_3D)
print(fmri_filepath)

#save T1w 3D pdf image
plotting.plot_img(sub13_01_3D)
plt.savefig("t1w.pdf", format="pdf", bbox_inches="tight")
...
plt.savefig("cormat_pre.pdf", format="pdf", bbox_inches="tight")
...
plt.savefig("connectome90%_pre.pdf", format="pdf", bbox_inches="tight")
...
#plot time serie for seed voxel
import matplotlib.pyplot as plt
...
display.add_markers(marker_coords=pcc_coords, marker_color='g',
                    marker_size=300)
plt.savefig("pcc_seed_to_voxel_cor_pre.pdf", format="pdf", bbox_inches="tight")
...
seed_to_voxel_correlations_fisher_z_img.to_filename(
    'pcc_seed_correlation_pre_z.nii.gz')